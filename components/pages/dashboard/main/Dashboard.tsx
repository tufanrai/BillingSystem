...
import { Table } from "../menuandtables/MenuAndTables";
...
export default function DashboardPage() {
  // query total tables and menu items.
  const { data: totalTable } = useQuery({
    queryKey: ["AllTables"],
    queryFn: getAllTable,
  });

  const { data: totalMenuItems } = useQuery({
  ...
  const ButtonCards: IButtonCard[] = [
    ...
    },
    {
      ...
      title: "Menu items",
      subtitle: (totalMenuItems || []).length ?? 0,
      url: "/dashboard/menuandtables",
    },
  ];
  ...
  const FinanceCards: IFinanceCard[] = [
    ...
    },
    {
      ...
      subtitle: "0",
      metaInfo: `Available ${
        (totalTable || []).filter((t: Table) => t.status === "Available")
          .length ?? 0
      }`,
      tag: (
